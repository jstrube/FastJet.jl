...
global const libfjcore = joinpath(dirname(@__FILE__), "usr/lib/libfjcore.so")
...
#    global libfjcore
#    if !isfile(libfjcore)
#        error("$(libfjcore) does not exist, Please re-run Pkg.build(\"LCIO\"), and restart Julia.")
#    end
#
#    if Libdl.dlopen_e(libfjcore, Libdl.RTLD_LAZY|Libdl.RTLD_DEEPBIND|Libdl.RTLD_GLOBAL) in (C_NULL, nothing)
#        error("$(libfjcore) cannot be opened, Please re-run Pkg.build(\"LCIO\"), and restart Julia.")
#    end
    ...
    if Libdl.dlopen_e(libfastjetwrap) in (C_NULL, nothing)
